...
<body class="ltr main-body leftmenu">
    ...
    <div class="">
        ...
        <div class="modal fade" id="guestSignOutModal" tabindex="-1" aria-labelledby="guestSignOutModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-primary" type="button" id="showRegistrationButton">Convert To
                            User</button>
                        <a class="btn btn-danger" href="{{ route('GuestLogout') }}">Sign Out</a>
                    </div>

                </div>
            </div>
        </div>
        <div class="modal fade" id="registrationFormModal" tabindex="-1" aria-labelledby="registrationFormModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form method="POST" action="{{ route('Convert2User') }}">
                            ...
                            <div class="d-flex-text-center">
                                <form id="registrationForm" method="POST" action="{{ route('Convert2User') }}"
                                    style="display: none">
                                    @csrf
                                    <div class="form-group text-start">
                                        <label>Name</label>
                                        <input class="form-control" placeholder="John Doe" type="text" name="name"
                                            value="{{ old('name') }}" required autofocus autocomplete="name">
                                    </div>

                                    <div class="form-group text-start">
                                        <label>Phone Number</label>
                                        <input class="form-control" placeholder="+1 (555) 123-4567" type="text"
                                            name="phone" value="{{ old('phone') }}" required autofocus
                                            autocomplete="phone">
                                    </div>
                                    <div class="form-group text-start">
                                        <label>Email</label>
                                        <input class="form-control" placeholder="john.doe@example.com" type="email"
                                    ...
                                    <button class="btn ripple btn-main-primary btn-block">Convert Account &
                                        Logout</button>
                                </form>
                            </div>
                        </form>
        ...
        <script>
            $(document).ready(function() {
                $('.main-notification-list .media').on('click', function() {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('user.mark.notification.read') }}",
                        data: {
                        ...
                        success: function(response) {
                            ...
                            var countElement = $('.main-notification-text').find('span.noti-count');
                            var countElement2 = $('.main-header-notification').find(
                                'span.badge');
                            var currentCount = parseInt(countElement.text());
                            if (currentCount > 0) {
                                countElement.text(currentCount - 1);
                            }
                            var currentCount2 = parseInt(countElement2.text());
                            if (currentCount2 > 0) {
                                countElement2.text(currentCount2 - 1);
